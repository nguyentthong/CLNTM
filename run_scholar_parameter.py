...
def main(args):
    ...
    model = train(model, network_architecture, train_X, train_labels, train_prior_covars, train_topic_covars, options, vocab, training_epochs=options.epochs, batch_size=options.batch_size, rng=rng, X_dev=dev_X, Y_dev=dev_labels, PC_dev=dev_prior_covars, TC_dev=dev_topic_covars)
...
def train(model, network_architecture, X, Y, PC, TC, options, vocab, batch_size=200, training_epochs=100, display_step=10, X_dev=None, Y_dev=None, PC_dev=None, TC_dev=None, bn_anneal=True, init_eta_bn_prop=1.0, rng=None, min_weights_sq=1e-7):
    # Train the model
    ...
    for epoch in range(training_epochs):
        ...
        if 'contrastive' in options.model.lower():
            options.output_dir = './outputs/parameter_scholar/contrastive_imdb_12_' + str(epoch) 
        else:
            options.output_dir = './outputs/parameter_scholar/imdb_12_' + str(epoch) 
        fh.makedirs(options.output_dir)
        print_and_save_weights(options, model, vocab, None, None)
