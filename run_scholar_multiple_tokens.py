...
def main(args, num_tokens):
    ...
    if '20ng' in input_dir:
        options.output_dir = './outputs/multiple_token_scholar/20ng_50_' + str(num_tokens)
    elif 'imdb' in input_dir:
        options.output_dir = './outputs/multiple_token_scholar/imdb_50_' + str(num_tokens)
    else:
        options.output_dir = './outputs/multiple_token_scholar/wiki_50_' + str(num_tokens)
    ...
    # create the model
    model = Scholar(network_architecture, alpha=options.alpha, learning_rate=options.learning_rate, init_embeddings=embeddings, update_embeddings=update_embeddings, init_bg=init_bg, adam_beta1=options.momentum, device=options.device, seed=seed, classify_from_covars=options.covars_predict, model=options.model, topk=num_tokens)
    ...
    print_and_save_weights(options, model, vocab, num_tokens, prior_covar_names, topic_covar_names)
...
def print_and_save_weights(options, model, vocab, num_tokens, prior_covar_names=None, topic_covar_names=None):
    ...
    if '20ng' in options.input_dir:
        if 'contrastive' in options.model.lower(): torch.save(model, './multiple_token_weights/20ng_50_' + str(num_tokens) + '.pt')
        else: torch.save(model, './multiple_token_weights/20ng_50.pt')
    elif 'imdb' in options.input_dir:
        if 'contrastive' in options.model.lower(): torch.save(model, './multiple_token_weights/imdb_50_' + str(num_tokens) + '.pt')
        else: torch.save(model, './multiple_token_weights/imdb_50.pt')
    else:
        if 'contrastive' in options.model.lower(): torch.save(model, './multiple_token_weights/wiki_50_' + str(num_tokens) + '.pt')
        else: torch.save(model, './multiple_token_weights/wiki_50.pt')
...
if __name__ == '__main__':
    num_token_list = [1,5,10,15,20,25,30]
    for i in range(len(num_token_list)):
        print(i)
        main(sys.argv[1:],num_token_list[i])
